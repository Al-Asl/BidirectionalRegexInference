...
bool rei::readFile(const std::string& fileName, std::vector<std::string>& pos, std::vector<std::string>& neg) {
 ...
 rei::OperationsCount  rei::countOpreations(const string& pattern) {
 ...
 int rei::calculateCost(const std::string& pattren, const unsigned short* costFunc) {
     auto counts = rei::countOpreations(pattren);
     int count = 0;
     count += counts.alpha * costFunc[0];
     count += counts.question * costFunc[1];
     count += counts.star * costFunc[2];
     count += counts.concat * costFunc[3];
     count += counts.alternation * costFunc[4];
     return count;
 }
