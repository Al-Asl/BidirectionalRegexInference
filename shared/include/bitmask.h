...
namespace rei
{
    ...
    public:
        ...
        HD static int getChuncksSize(int bitsSize) {
            return (bitsSize - 1) / (sizeof(T) * 8) + 1;
        }

        static constexpr int chunckBitSize = sizeof(T) * 8;

        HD BitmaskView(T* ptr, int size) : ptr(ptr), size(size) {}

        HD BitmaskView() : ptr(nullptr), size(0) {}

        HD bool isValid() const { return ptr != nullptr; }

        HD BitmaskView& setChunck(T value, int idx) {
            ptr[idx] = value;
            return *this;
        }

        HD uint64_t getChunck(int idx) const {
            return ptr[idx];
        }

        HD BitmaskView& toggleBit(int bitIndex) {
        ...
        HD BitmaskView& setBitOn(int bitIndex) {
            int eIdx = bitIndex / chunckBitSize;
            int bIdx = bitIndex % chunckBitSize;
            ptr[eIdx] |= (1ULL << bIdx);
            return *this;
        ...
        HD BitmaskView& setBitOff(int bitIndex) {
            int eIdx = bitIndex / chunckBitSize;
            int bIdx = bitIndex % chunckBitSize;
            ptr[eIdx] &= ~(1ULL << bIdx);
            return *this;
        ...
        HD bool getBit(int bitIndex) const {
            int eIdx = bitIndex / chunckBitSize;
            int bIdx = bitIndex % chunckBitSize;
            return (ptr[eIdx] >> bIdx) & 1ULL;
        }

        HD BitmaskView& operator|=(const BitmaskView& other) {
        ...
        HD inline bool intersects(const BitmaskView& other) const {
            for (int i = 0; i < size; ++i) {
        ...
        HD bool containsAll(const BitmaskView& other) const {
            for (int i = 0; i < size; i++) {
        ...
        HD bool containsNone(const BitmaskView& other) const {
            for (int i = 0; i < size; i++) {
                if ((ptr[i] & other.ptr[i]) != 0) {
                    return false;
                }
        ...
        HD Pair<uint64_t> getHash128() const {
        ...
        HD void copyTo(uint64_t* destBuffer) const {
#ifdef __CUDA_ARCH__
            for (int i = 0; i < size; i++)
                destBuffer[i] = ptr[i];
#else
        ...
        HD void copyTo(BitmaskView& bitmask) const {
#ifdef __CUDA_ARCH__
            for (int i = 0; i < size; i++)
                bitmask.ptr[i] = ptr[i];
#else
        ...
        HD BitmaskView& copy(const uint64_t* destBuffer) {
#ifdef __CUDA_ARCH__
            for (int i = 0; i < size; i++)
                ptr[i] = destBuffer[i];
#else
        ...
        HD BitmaskView& copy(const BitmaskView& bitmask) {
#ifdef __CUDA_ARCH__
            for (int i = 0; i < size; i++)
                ptr[i] = bitmask.ptr[i];
#else
        ...
        std::vector<int> getBits(int size) const {
            std::vector<int> res;
            ...
            for (int chunckIdx = 0; chunckIdx < size; ++chunckIdx) {
                ...
                for (int bitIdx = 0; bitIdx < chunckBitSize; ++bitIdx) {
                    int gbitIdx = chunckIdx * 64 + bitIdx;
                    if (gbitIdx >= size) return res;
                    if (chunk & (1ULL << bitIdx))
                        res.push_back(gbitIdx);
                }
            }
        ...
        HD bool operator==(const BitmaskView& other) const {
            for (int i = 0; i < size; ++i) {
        ...
        HD BitmaskView& operator>>=(int shift) {
        ...
        HD int getSize() const { return size; }
    ...
    public:
        ...
        HD BitmaskBufferView() = default;

        HD BitmaskBufferView(T* ptr, int elementsCount, int numChunkPerElement, bool isFull = false)
            : ptr(ptr), elementsCount(elementsCount), numChunkPerElement(numChunkPerElement), _count(isFull ? elementsCount : 0) {
        }

        HD BitmaskView<T> at(int index) const {
            return  BitmaskView(ptr + index * numChunkPerElement, numChunkPerElement);
        }

        HD BitmaskView<T> operator[](int index) const {
            return BitmaskView(ptr + index * numChunkPerElement, numChunkPerElement);
        }

        HD T* data() { return ptr; }
        HD int size() const { return elementsCount; }
        HD int getNumChuncksPerElement() const { return numChunkPerElement; }

        HD BitmaskBufferView getView(int start, int elementsCount, bool isFull = false) {
